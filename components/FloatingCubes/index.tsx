...
export default function FloatingCubes(props: { config?: Partial<FloatingCubesConfig>, camera?: number[], controls?: boolean, background?: string }) {
	const config = {...DefaultConfig, ...props.config}
	const cubes: Cube[] = []

	for(let i = 0; i < config.amount; i++) {
		let scale = Math.random() * (config.maxScale - config.minScale) + config.minScale
		...
		if(isValidPosition(position, cubes)) {
			cubes.push({
				position: position,
				scale:  scale,
				component: <Cube key={i}
					config={{
						position: position,
						scale: scale,
						reverse: cubes.length % 2 == 0 ? true : false,
						rotation: Math.random() * (0.0025 - -0.0025) + -0.0025 * config.rotation,
						range: config.range,
						color: config.color,
						speed: config.speed
					}}
    	...
    	<Canvas color={props.background || "red"}>
			...
			{props.controls ? 
				...
				<PerspectiveCamera makeDefault position={
					props.camera ? new THREE.Vector3(props.camera[0], props.camera[1], props.camera[2]) : [0, 0, 10]
				} />
			}
